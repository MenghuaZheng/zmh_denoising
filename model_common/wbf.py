...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, shift_dir='',
        ...
        print("self.input_res ", self.input_resolution)
        print("self.num_head ", self.num_heads)
        print("win_szie ", self.window_size)
        print("shift_dir,", self.shift_dir)
        print("shift_dir ", self.shift_size)
        print("atten_mask", attn_mask)
        '''
...
class WBF(nn.Module):
    ...
    def __init__(self, dim, input_szie, num_heads, window_size, layer, patch_dim,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, ape=False):

        super().__init__()
        assert input_szie % patch_dim == 0
        assert window_size % patch_dim == 0
        assert len(layer) if isinstance(layer, list) else 1 == len(drop_path) if isinstance(drop_path, list) else 1

        self.dim = dim*patch_dim*patch_dim
        self.input_resolution = (input_szie//patch_dim, input_szie//patch_dim)
        self.win_resolution = window_size//patch_dim
        self.shift_resolution = self.win_resolution//2
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=self.dim, input_resolution=self.input_resolution,
                                 num_heads=num_heads, window_size=self.win_resolution,
                                 shift_size=self.shift_resolution,
                                 shift_dir=layer[i] if isinstance(layer, list) else layer,
                                 mlp_ratio=mlp_ratio,
                                 qkv_bias=qkv_bias, qk_scale=qk_scale,
                                 drop=drop, attn_drop=attn_drop,
                                 drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                 norm_layer=norm_layer)
            for i in range(len(layer) if isinstance(layer, list) else 1)])

        # absolute position embedding
        if self.ape:
            self.absolute_pos_embed = \
                nn.Parameter(torch.zeros(1, self.input_resolution[0] * self.input_resolution[1], self.dim))
