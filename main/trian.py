...
if args.n_colors == 3:
    ...
    if args.sigma == 200.0:  # 使用的是真实噪声
        ...
        args.train_dataset = "real_dataset"  # 训练集只有有一个
    ...
    My_Test_Dataset = Art_nosie_Dataset
    My_Train_Dataset = GenDataset
    # My_Dataset = GenDataset

    if args.test_dataset == "CBSD68":  # BSD68是默认值
        args.test_dataset = "BSD68"
        print("n_color is 1, Training in BSD432, evaluate in BSD68")
    else:
...
'''
train_dir = os.path.join(args.dir_data, 'train', args.train_dataset)
test_dir = os.path.join(args.dir_data, 'test', args.test_dataset)
save_model_dir = os.path.join(args.dir_model)
save_state_dir = os.path.join(args.dir_state)
save_loss_dir = os.path.join(args.dir_loss)
save_test_dir = os.path.join(args.dir_test_img)
'''
...
def train(args):
    ...
    # milestone = [(15 - args.start_epoch)*3,
    #              (22 - args.start_epoch)*3,
    #              (24 - args.start_epoch)*3,
    #              (26 - args.start_epoch)*3,
    #              (28 - args.start_epoch)*3,
    #              (30 - args.start_epoch)*3,
    #              (31 - args.start_epoch)*3,
    #              190]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # ======================================save=============================================================
        if args.flag == 0:
            if epoch % 1 == 0: # 每5个Epoch保存一次模型文件和状态
