...
'''
def augment_img_plus(img, mode=0):
    seq1 = iaa.Sequential([
        iaa.Fliplr(0.6),  # 对60%的图像进行镜像翻转
        iaa.Flipud(0.6),  # 对60%的图像做左右翻转
    ])
    ...
    if mode == 0:
        ...
        seq = iaa.Sequential([
            ...
            iaa.Flipud(0.5),  # 对20%的图像做左右翻转
            iaa.ChannelShuffle(0.35),  # 打乱通道
        ])

        img1 = seq1.augment_image(img)
        img2 = seq.augment_image(img1)
        return img2
        ...
        seq = iaa.SomeOf(1, [
            ...
            iaa.BlendAlphaSomeColors(iaa.MultiplySaturation(0.5),
    ...
    # 6. Flipud
    # 7. Affine: 仿射变换
    # 8. PiecewiseAffine: 变形
    # 9. PerspectiveTransform
    # 10. Rot90
    ## 11. BlendAlPha
    # 12. BlendAlphaSomeColors
    # 13. WithColorspace
    # 14. WithBrightnessChannels
    # 15. ChangeColorTemperature
...
def rgb2g(rgb_dir, g_dir):  # 将RGB图像转化为Grey图像
    # os.listdir() 方法用于返回指定的文件夹包含的文件或文件夹的名字的列表。这个列表以字母顺序
    filelist = os.listdir(rgb_dir)

    for img_name in filelist:
        img_path = os.path.join(rgb_dir, img_name)
        save_path = os.path.join(g_dir, img_name)

        if not os.path.exists(g_dir):  # 没有问该文件夹，则创建该文件夹
            os.makedirs(g_dir)

        rgb_image = cv2.imread(img_path)

        g_image = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2GRAY)

        cv2.imwrite(save_path, g_image)


if __name__ == '__main__':
    rgb2g("../data/images/train/CBSD432", "../data/images/train/BSD432")
